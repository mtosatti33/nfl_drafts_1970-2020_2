...
uses
  Classes, SysUtils, DBGrids, ZDataset, Graphics;
...
begin
  ...
  begin
    ...
    if FQuery.FieldByName('team_av').IsNull then
  	  tm_av:=0 else
      tm_av:=FQuery.FieldByName('team_av').Value;
    ...
    starter :=FQuery.FieldByName('years_starter').Value;
  end;
  ...
  0:
    begin
      if FQuery.RecordCount <> 0 then
        if FDatacol = 3 then
        begin
          FGrid.Canvas.Font.Color:=FQuery['secondary_colors'];
          FGrid.Canvas.Brush.Color:=FQuery['primary_colors'];
        end;
    end;
  // AV Margin
  1:
